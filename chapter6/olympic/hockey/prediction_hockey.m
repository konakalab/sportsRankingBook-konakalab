...
for n1=1:size(plotData,1)
    for n2=1:size(plotData,2)
        ...
        c=Opt.min_pWin;
        tmp_pWin=glmval(mdl.win, taRating-tbRating,'logit');
        plotData(n1,n2)=c+(1-2*c)*tmp_pWin;
        plotData2=[plotData2;n2 n1 c+(1-2*c)*tmp_pWin];
    end
end
plotData;
imagesc(plotData)
...
saveas(gca,['prediction_h2h_Olympic_2024_' Opt.sportName '_' Opt.sexStr],'png')
saveas(gca,['prediction_h2h_Olympic_2024_' Opt.sportName '_' Opt.sexStr],'pdf')
...
for k=1:Opt.nSeasons
    ...
    for groupName=unique(tbl_teams.Group)'
        ...
        for n1=1:size(matchTable,1)
            ...
            pWin=glmval(mdl.win,rDiff,'logit')+c;   % 最小勝率を考慮した予測勝率
            pLose=glmval(mdl.lose,rDiff,'logit')+c;   % 最小勝率を考慮した予測敗率
            pDraw=1-pWin-pLose; % 引き分け確率
    ...
    finalStandings(tmpStTable(5:end,1),k)=9;
    finalStandings(tmpStTable(5:end,2),k)=9;
    % QF(ベスト8)
    tournament.SF=[];
    D=mdl.win(2);
    for n1=1:4
...
set(gca,'YDir','reverse', 'FontName','メイリオ', 'FontSize',10, ...
    ...
    'YTickLabel', [char(tbl_teams.IOCCode) ...
...
saveas(gcf,['prediction_final_' Opt.sportName '_' Opt.sexStr '_' ],'png');
saveas(gcf,['prediction_final_' Opt.sportName '_' Opt.sexStr '_' ],'pdf');

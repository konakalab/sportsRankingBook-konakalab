...
load('../tbl_IOCCode.mat','tbl_IOCCode');
...
for k=1:Opt.nSeasons
    ...
    for groupName=unique(tbl_teams.Group)'
        ...
        for n1=1:size(matchTable,1)
            ...
            pWin=c+(1-2*c)/(1+exp(-mdl(2)*(rDiff)));    % 最小勝率を考慮した予測勝率
            pLose=1-pWin;
            pDraw=1-pWin-pLose;
            ...
            if tmpRnd<pWin
                ...
                pointsInGS(taNum,k)=pointsInGS(taNum,k)+2+0.001*taRating*rand();
                pointsInGS(tbNum,k)=pointsInGS(tbNum,k)+1+0.001*tbRating*rand();
            elseif tmpRnd<pWin+pDraw
                % 引き分けの場合

            else
                % チームBが勝利の場合
                pointsInGS(taNum,k)=pointsInGS(taNum,k)+1+0.001*taRating*rand();
                pointsInGS(tbNum,k)=pointsInGS(tbNum,k)+2+0.001*tbRating*rand();
            end
        end
    end
    ...
    tmpStTable;
    pointsInGS(tmpStTable);
    [val,ind]=sort(pointsInGS( tmpStTable(2,:),k),'descend');
    groupD=[tmpStTable(1,:)'; tmpStTable(2,ind(1))];
    groupE=[tmpStTable(2,ind(2:3))'];
    [val,ind]=sort(pointsInGS( tmpStTable(3,:),k),'descend');
    groupE=[groupE;tmpStTable(3,ind(1:2))'];
    ...
    while true
        ...
        tbl_teams.Group( [groupD groupE]);
        if sum( ans(:,1)==ans(:,2))==0
            break
        end
    end
    tbl_teams.Group([groupD groupE]);
    
    tournament.QF=[groupD(1)
        groupE(1)
        groupD(2)
        groupE(2)
        groupD(3)
    ...
    finalStandings(setdiff(1:12, tournament.QF),k)=9;
    % QF(ベスト8)
    tournament.SF=[];
    D=mdl(2);
    for n1=1:4
        ...
        [taRating tbRating];
        if  rand()<c+(1-2*c)/(1+exp(-D*(taRating-tbRating)))
    ...
    for n1=1
        ...
        if  rand()<c+(1-2*c)/(1+exp(-D*(taRating-tbRating)))
            ...
            finalStandings(tbNum,k)=3;
            finalStandings(taNum,k)=4;
...
legend({'Gold','Silver','Bronze','4th','QF','GS'},'Location','northoutside','Orientation','horizontal');
